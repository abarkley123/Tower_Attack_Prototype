...
function determineNextPoint(p) {
    // this is a "constant"  - representing 10px motion per "time unit"
    const bulletSpeed = p.topSpeed;
    const target = p.target;
    const targetCenter = getCenter(target, target.radius/2, target.radius/2);
    const bulletCenter = getCenter(p, p.width, p.length);
    var enemyVec = vec_sub(targetCenter, bulletCenter);
...
function getCenter(e, width, height) { 
  return {
    x: e.pos.x+(width/2), 
    y: e.pos.y+(height/2) 
  }; 
...
function neighbors(grid, col, row, val) {
 ...
 //   console.log((col-1) + ' ' + row);
    try {
        ...
        console.log("Couldn't resolve neighbour: " + NoSuchTileException);
    }
