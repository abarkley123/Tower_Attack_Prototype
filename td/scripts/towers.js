...
tower.gun = {
    ...
    drawBase: function() {
        stroke(this.border);
        fill(this.color);
        rect(this.pos.x-this.length * 1.2 * ts / 2, this.pos.y-this.length * 1.2 * ts / 2, this.length * 1.2 * ts, this.length * 1.2 * ts);
    },
    ...
    drawBarrel: function() {
        ...
        fill([23,32,42]);
        ellipse(0, 0, this.radius/2 * ts, this.radius/2 * ts);
    },
    ...
    upgrades: [
        {
            ...
            onHit: function(e) {
                var blastRadius = 1;
                var inRadius = getInRange(e.pos.x, e.pos.y, blastRadius, units);
                noStroke();
                fill(this.color[0], this.color[1], this.color[2], 127);
    ...
    upgrades: [
        {
            ...
            onHit: function(e) {
                ...
                for (var i = 0; i < segs; i++) {
                    ...
                    var inRadius = getInRange(x, y, blastRadius, units);
                    if (showEffects) {
...
tower.tesla = {
    ...
    onAim(e) {
        ...
        while (dmg > 1) {
            ...
            targets.push(last);
            var next = getNearest(units, last.pos, targets);
